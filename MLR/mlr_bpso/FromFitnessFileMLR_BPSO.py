...
#Ahmad Hadaegh: Modified  on: July 16, 2013
def validate_model(generation, model, fileW, population, TrainX, TrainY, ValidateX, ValidateY, TestX, TestY):
	...
	Yhat_cv         = cv_predict(X_train_masked, TrainY, model)
	...
	#Yhat_fitness = append(Yhat_cv, Yhat_validation) # MARK: use of cross validation
        Yhat_fitness = append(Yhat_train, Yhat_validation) # MARK: use of cross validation
        Yhat_fitness = append(Yhat_fitness, Yhat_test)
        ...
        if predictive and ((q2_loo < 0.5) or (r2pred_validation < 0.5) or (r2pred_test < 0.5)):
            ...
            print "ending the program because predictive is: ", predictive
            continue
               
        # Standard error of estimate
        s               = sdep(TrainY, Yhat_train)
        sdep_validation = sdep(ValidateY, Yhat_validation)
        sdep_test       = sdep(TestY, Yhat_test) # was do training
...
'''def write(model,fileW, trackDesc, trackIdx, trackFitness, trackModel, trackR2, \
    ...
    for key in trackFitness.keys():
        fileW.writerow([trackDesc[key], trackIdx[key], trackFitness[key], trackModel[key], \
            ...
            trackSDEPTrain[key], trackSDEPValidation[key], trackSDEPTest[key], \
