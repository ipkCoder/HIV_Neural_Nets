...
def rescaleTheData(TrainX, TrainY, ValidateX, ValidateY, TestX, TestY):
    ...
    for i in range(0, ValidateY.shape[0]):
        ValidateY[i] = (ValidateY[i] - TrainYMean)/sqrt(TrainYVar)
...
TrainX, TrainY, ValidateX, ValidateY, TestX, TestY = rescaleTheData(TrainX, TrainY, ValidateX, ValidateY, TestX, TestY)

ffn = FeedForwardNetwork()

# create layers
inLayer = LinearLayer(TrainX.shape[1], name="input")
...
ds = SupervisedDataSet(TrainX.shape[1],1)
...
for i in range(TrainX.shape[0]):
	# print data[i]
	# print y[i]
	ds.addSample(TrainX[i], TrainY[i])
...
print "\n"
for i in range(TrainX.shape[0]):
	print ffn.activate(TrainX[i]), TrainY[i]
