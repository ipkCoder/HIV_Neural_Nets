...
def cv_predict(set_x, set_y, val_x, val_y, model, neurolabANN):
...
def validate_model(model, fileW, population, TrainX, TrainY,\
    ...
    for i in range(numOfPop):
        ...
        with Timer() as t:
            ...
            try:
                with Timer() as t0:
                    Yhat_cv = cv_predict(X_train_masked, TrainY, X_validation_masked, ValidateY, model, net)    # Cross Validation
            finally:
                print "Time to predict cross validate for {}: {} sec".format(i, t0.interval)
            ...
            # neurolab testing...
            inp_xtrain                 = X_train_masked;
            Yhat_train                 = net.sim(inp_xtrain)
            Yhat_train                 = np.array([Yhat_train[i][0] for i in range(0,len(Yhat_train))])
            ...
            trackFitness[idx]          = fitness[fitnessAcc]
            fitnessAcc                +=1
